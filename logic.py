...
class StrategyEngine:
    ...
    # --- MODULE 3: WAVES & HISTORICAL VOL ---
    def generate_spectrogram_data(self, ticker):
        """
        Returns: Freqs, SpecDates, Intensity, PriceDates, Prices, AND HistoricalVol
        """
        try:
            # 1. Fetch Data
            data = yf.download(ticker.split(" ")[0], period="2y", interval="1d", progress=False)
            if data.empty: return None, None, None, None, None, None # Return 6 Nones
            ...
            # 3. Calculate Historical Volatility (30-Day Rolling Annualized)
            # Log Returns for Vol calculation
            log_ret = np.log(data['Close'] / data['Close'].shift(1))
            # Rolling Std Dev * Sqrt(252) for Annualized
            hist_vol = log_ret.rolling(window=30).std() * np.sqrt(252)
            hist_vol = self.to_scalar_array(hist_vol.fillna(0))
            
            return f, spec_dates, Sxx, price_dates, prices, hist_vol # Return 6 items
            
        except Exception as e:
            print(f"Spectrogram Error: {e}")
            return None, None, None, None, None, None

    # --- MODULE 4: OPTIONS (Current Snapshot) ---
    def get_options_analytics(self, ticker):
        try:
            ...
            atm = calls[(calls['strike'] > curr*0.95) & (calls['strike'] < curr*1.05)]
            iv = atm['impliedVolatility'].mean() if not atm.empty else 0
            
            hist = tk.history(period="1mo")
            hv = (np.log(hist['Close']/hist['Close'].shift(1)).std() * np.sqrt(252)) if not hist.empty else 0
